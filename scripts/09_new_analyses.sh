...
do
  for i in {1..4}
  do
    paste <(sed '1i ind_ID' inds_for_newhybrids_$NAME.txt) <(cut --complement -f1-2 run_nh/$NAME/run${i}/${NAME}_run${i}_PofZ.txt | sed '1d' | cat GtypeFreqExtBxs_headers.txt -) > run_nh/ess/${NAME}_nh_rawlabels_run${i}.txt
  done
done < speciespairs.txt

conda activate R_newhybrids
Rscript ~/qmac/scripts/newhybrids_esscalc.R "../figures/" --vanilla
conda deactivate
