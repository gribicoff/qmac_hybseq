...
# 7707728 (vascular plants)

sppkeys <- c("2878213","2879737","2878470","8879961","2879947")
spp <- c("mac","alb","bic","ste","mue")

# download target group background points from GBIF - make sure GBIF API key is set
...
bg.df <- bg_dl %>%
  ...
  dplyr::filter(dplyr::between(long, -110, -58) & dplyr::between(lat, 23, 60))

# print number of background occurrences

nrow(bg.df)

# write csv file of all background points - NOT filtered on per-species basis

write.csv(file = "unfiltered_targetbgpts.csv", bg.df, row.names = FALSE, quote = FALSE)

# write shapefile of background points

bg.shp <- st_as_sf(bg.df, coords = c("long","lat"), crs = 4326)
st_write(bg.shp, "unfiltered_targetbgpts.shp")
